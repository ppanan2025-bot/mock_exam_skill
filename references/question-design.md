...
- Macro / program / pseudocode definitions belong in `code`, not in the stem. English prompt in `stem`, indented listing in `code`, follow-up sentence in `stem_after` ("Which macro statement does this define?"). Choices use the course operators: `left % right`, `left // right`, `left * right`, `right - left`. Do not write "remainder", "mod", "divides", "product", or "times" as the option text. Never flatten a loop into `rem = left; while rem >= right: { rem = rem - right }`.
